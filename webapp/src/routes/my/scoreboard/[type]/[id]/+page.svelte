...
<script lang="ts">
import { m } from '@/i18n';
import { OTelDetails } from '$lib/scoreboard';
...
<td class="table-cell">
<span class="status-badge" class:status-ok={span.status.code === 'OK'} class:status-error={span.status.code !== 'OK'}>
{span.status.code}
</span>
</td>
...
return rate !== undefined && rate !== null ? `${rate}%` : m.not_available();
})()}
</td>
</tr>
{/each}
