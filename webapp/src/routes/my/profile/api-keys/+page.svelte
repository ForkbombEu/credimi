...
<script lang="ts">
	...
	async function generateApiKey() {
		...
		try {
			...
			generatedApiKeyName = validation.data.name;
			apiKeyName = '';
...
<div class="space-y-6">
	...
	<div>
		...
		<T class="text-muted-foreground">{m.Generate_a_new_API_key_for_accessing_the_services()}</T>
	</div>
	<div class="flex flex-col gap-4">
		...
		<Button
			...
			disabled={isLoading || !apiKeyNameSchema.safeParse({ name: apiKeyName }).success}
		>
	...
	<div>
		...
		<T class="text-muted-foreground">{m.Here_are_your_active_API_keys()}</T>
	</div>
	<Card.Content>
		{#if $currentUser?.id}
			<CollectionManager
