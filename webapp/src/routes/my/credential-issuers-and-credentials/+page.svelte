...
{#snippet infoLink(props: { label: string; href?: string })}
	<div class="flex items-center gap-1">
		...
		{#if props.href}
			<A class="link-sm block w-0 grow truncate" target="_blank" href={props.href}>
				{props.href}
			</A>
		{:else}
...
<style lang="postcss">
	:global(.link-sm) {
		@apply cursor-pointer !text-gray-400 underline underline-offset-2;
	}
</style>
