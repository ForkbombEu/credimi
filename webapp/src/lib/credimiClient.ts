...
export type WorkflowExecutionConfigWithMetadata = z.infer<
	typeof WorkflowExecutionConfigWithMetadataSchema
>;
...
export const GenerateApiKeyResponseSchemaSchema = z.object({
	api_key: z.string()
});
export type GenerateApiKeyResponseSchema = z.infer<typeof GenerateApiKeyResponseSchemaSchema>;

export const AuthenticateApiKeyResponseSchemaSchema = z.object({
	message: z.string(),
	token: z.string()
});
export type AuthenticateApiKeyResponseSchema = z.infer<
	typeof AuthenticateApiKeyResponseSchemaSchema
>;
...
export class CredimiClient {
	 ...
	 */
	async listMyCheckRuns(
		input: { checkId: string },
		options?: ClientOptions
	): Promise<ListMyCheckRunsResponse> {
	 ...
	 */
	async getMyCheckRun(
		input: { checkId: string; runId: string },
		options?: ClientOptions
	): Promise<GetMyCheckRunResponse> {
	 ...
	 */
	async getMyCheckRunHistory(
		input: { checkId: string; runId: string },
		options?: ClientOptions
	): Promise<GetMyCheckRunHistoryResponse> {
	 ...
	 */
	async rerunMyCheck(
		input: ReRunCheckRequest & { checkId: string; runId: string },
		options?: ClientOptions
	 ...
	 */
	async cancelMyCheckRun(
		input: { checkId: string; runId: string },
		options?: ClientOptions
	): Promise<CancelMyCheckRunResponse> {
	 ...
	 */
	async exportMyCheckRun(
		input: { checkId: string; runId: string },
		options?: ClientOptions
	): Promise<ExportMyCheckRunResponse> {
	 ...
	 */
	async myCheckLogs(
		input: { checkId: string; runId: string } & { action?: string },
		options?: ClientOptions
	): Promise<ChecksLogsResponse> {
	 ...
	 */
	async terminateMyCheckRun(
		input: { checkId: string; runId: string },
		options?: ClientOptions
	): Promise<TerminateMyCheckRunResponse> {
	...
	async generateApiKey(
		...
		options?: ClientOptions
	): Promise<GenerateApiKeyResponseSchema> {
		const path = `/api/apikey/generate`;

		var opt: SendOptions = { method: 'POST' };

		opt.body = GenerateApiKeyRequestSchemaSchema.parse(input);
		if (options) {
			opt = { ...opt, ...options };
		}
		...
		try {
			const result = await this.pb.send(path, opt);
			return GenerateApiKeyResponseSchemaSchema.parse(result);
		} catch (error: any) {
			if (error && error.data) {
				const parsedError = APIErrorSchema.safeParse(error.data);
				if (parsedError.success) {
	 ...
	 */
	async authenticateApiKey(options?: ClientOptions): Promise<AuthenticateApiKeyResponseSchema> {
		...
		try {
			const result = await this.pb.send(path, opt);
			return AuthenticateApiKeyResponseSchemaSchema.parse(result);
		} catch (error: any) {
			if (error && error.data) {
				const parsedError = APIErrorSchema.safeParse(error.data);
				if (parsedError.success) {
