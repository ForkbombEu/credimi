...
<script lang="ts">
	...
	function get_base_extensions(): Extension[] {
		...
		if (lineNumbers)
			extensions.push(lineNumbersExt(lineNumbers === true ? undefined : lineNumbers));
		...
		if (highlight.selectionMatches) {
			extensions.push(
				highlightSelectionMatches(
					highlight.selectionMatches === true ? undefined : highlight.selectionMatches
				)
			);
		}
		if (history) {
		...
		if (drawSelection) {
			extensions.push(
				drawSelection === true ? drawSelectionExt() : drawSelectionExt(drawSelection)
			);
		}
		...
		if (bracketMatching) {
			extensions.push(
				bracketMatchingExt(bracketMatching === true ? undefined : bracketMatching)
			);
		}
		if (closeBrackets) {
		...
		if (crosshairCursor) {
			extensions.push(
				crosshairCursorExt(crosshairCursor === true ? undefined : crosshairCursor)
			);
		}
