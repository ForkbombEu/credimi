...
* WIP [#A] Pipeline queue limit per runner and owner namespace
- Effort :: M
...
** WIP [#A] Add `organizations.max_pipelines_in_queue` (read at enqueue time)
...
** TODO [#A] Enforce the per-runner limit inside the existing runner semaphore workflow
...
** TODO [#A] Enforce the limit at enqueue time (API); keep rollback and pipeline start unchanged
...
** TODO [#B] Alternative considered: “pre-gate” workflow in org namespace + Temporal rate limits
