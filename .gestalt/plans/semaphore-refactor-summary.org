...
* DONE [#B] Copilot review follow-ups (distilled)
- Effort :: S
...
** DONE [#B] Ensure a single start failure doesn’t stall queue processing
...
** DONE [#B] Clarify rollback runner list semantics (attempted vs succeeded)
...
** DONE [#C] Confirm `createPipelineExecutionResultWithRetry` loop is bounds-safe (false positive)
