...
* WIP [#A] Add a scheduled “enqueue-only” workflow (no pipeline execution)
- Effort :: M
...
* TODO [#A] Introduce a reusable “enqueue run ticket” activity (ports/adapters)
- Effort :: M
...
** TODO [#A] Implement enqueue activity + shared aggregation
...
* TODO [#A] Switch schedule creation to the new scheduled-enqueue workflow
- Effort :: M
...
** TODO [#A] Update schedule handler wiring and enrichment decoding
...
* TODO [#A] Remove the “scheduled run” mode from the pipeline workflow input
- Effort :: M
...
** TODO [#A] Delete `Scheduled` flag + `handleScheduledRun` implementation
...
* TODO [#A] Eliminate “acquire permit and wait inside pipeline workflow” logic
- Effort :: L
...
- Notes ::
  - Keep backwards compatibility for *already running* legacy workflows by not removing the old permit activities immediately; remove usage first, then clean up in a follow-up once old runs have drained.

** TODO [#A] Make non-semaphore mobile runs fail fast (no waiting)
