...
* WIP [#A] Show queued (not-started) pipeline runs in listings + cancel from queue
- Effort :: M
...
** TODO [#A] Add an API-side queued-run aggregator (ports/adapters boundary)
...
** TODO [#A] Extend `/api/pipeline/list-workflows` to include queued tickets per pipeline
...
** TODO [#A] Extend `/api/compliance/checks` (and `/api/my/checks`) list to include queued pipeline tickets
...
** TODO [#A] Update webapp lists to render queued rows and cancel from queue
