...
** WIP [#A] Record current generation entrypoints and outputs
...
** TODO [#A] Identify special cases in routes and schemas
...
** TODO [#A] Delete TS template and code paths
...
** TODO [#B] Ensure `go:generate` remains valid
...
** TODO [#A] Choose integration strategy (reflector-first, minimal manual patching)
...
** TODO [#A] Map route metadata into `OperationContext`
...
** TODO [#A] Generate request parameters (path/query) without touching handlers
...
** TODO [#A] Generate responses (success + default error)
...
** TODO [#B] Preserve (or intentionally improve) auth behavior
...
** TODO [#B] Implement YAML serialization and file writing
...
** TODO [#B] Ensure no `$ref` rewriting is needed
...
** TODO [#B] Add/adjust minimal tests for the generator logic (optional but recommended)
