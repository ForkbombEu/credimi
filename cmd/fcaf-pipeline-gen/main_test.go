...
func TestGenerateCompleteFCAFPipeline(t *testing.T) {
	...
	require.Len(t, definition.Steps, 659)
	...
	require.True(t, ok)
	require.Len(t, stringSlice(with["test_ids"]), 581)
	...
	require.Len(t, with["pipeline_outputs"], 184)
...
func TestGenerateHappyFlowFCAFPipeline(t *testing.T) {
	...
	require.Len(t, stringSlice(with["test_ids"]), 381)
	require.NotContains(
		t,
		stringSlice(with["test_ids"]),
