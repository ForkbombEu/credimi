...
func TestRegistryFactoriesAreWellFormed(t *testing.T) {
	for key, factory := range Registry {
		...
		require.True(t, factory.Kind == TaskActivity || factory.Kind == TaskWorkflow, "invalid kind for %s", key)
...
func requireOutputKindValid(t *testing.T, kind workflowengine.OutputKind, key string) {
	require.GreaterOrEqual(t, int(kind), int(workflowengine.OutputAny), "invalid output kind for %s", key)
	require.LessOrEqual(t, int(kind), int(workflowengine.OutputBool), "invalid output kind for %s", key)
}
