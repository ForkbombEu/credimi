...
func TestGetPlaceholders_MultipleReaders(t *testing.T) {
	reader1 := strings.NewReader(
		"Hello {{ credimiPlaceholder \"name\" \"123\" \"name_label\" \"name_desc\" \"string\" \"\" }}",
	)
	reader2 := strings.NewReader(
		"Age: {{ credimiPlaceholder \"age\" \"456\" \"age_label\" \"age_desc\" \"number\" \"\" }}",
	)
	reader3 := strings.NewReader(
		"Hello {{ credimiPlaceholder \"name\" \"123\" \"name_label\" \"name_desc\" \"string\" \"\" }} again",
	)
...
func TestGetPlaceholders_ComplexTemplate(t *testing.T) {
	...
	if !strings.Contains(content, "{{ .testalias }}") ||
		!strings.Contains(content, "{{ .client_id }}") {
	...
	for _, field := range fields {
		if expectedCredimiID, exists := expectedFields[field.FieldName]; !exists ||
			field.CredimiID != expectedCredimiID {
