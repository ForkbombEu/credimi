...
// Package reportgeneration prepares presentation data and PDFs for stored FCAF reports.
...
// AttachPresentation builds presentation data using the screenshots stored on record.
func AttachPresentation(app core.App, record *core.Record, report *engine.Report) {
	if report == nil {
		return
	}
	report.AttachPresentation(pipelineresults.ComputeMaestroScreenshotURLs(app, record))
}
...
func EnrichReportJSON(app core.App, record *core.Record, rawJSON []byte) ([]byte, *engine.Report, error) {
	...
	AttachPresentation(app, record, &report)
	root["presentation"] = report.Presentation
	enriched, err := json.MarshalIndent(root, "", "  ")
	if err != nil {
		return nil, nil, fmt.Errorf("encode FCAF report: %w", err)
	}
...
// LoadPipelineFCAFReportImages loads report screenshots stored on a pipeline result.
...
func resolvePipelineNameFromRecord(record *core.Record, fallback string) string {
