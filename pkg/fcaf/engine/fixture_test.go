	...
	cat := &catalog.Catalog{
		Tests: map[string]dsl.TestDefinition{
			"fixture-test": {
				...
				Evidence: map[string]dsl.EvidenceBinding{
					"value": {From: "pipeline.pid.outputs.value"},
				},
				Assertions: []dsl.AssertionDefinition{
					{ID: "value-present", Validator: "evidence.present", Input: "evidence.value"},
				},
		...
		Preconditions: map[string]dsl.PreconditionDefinition{
			"pipeline.pid": {
				...
				Kind:       "pipeline",
				PipelineID: "real-pid",
				Fixtures:   map[string]string{"beta_capture": "beta-pid"},
				Outputs: map[string]dsl.OutputDefinition{
					"value": {Path: "$.output.value", Decoder: "raw"},
				},
			},
		},
	}
	for _, tc := range []struct{ name, fixture, pipeline string }{{"default", "eudiw", "real-pid"}, {"beta", "beta_capture", "beta-pid"}} {
		t.Run(tc.name, func(t *testing.T) {
			...
			report, err := engine.ExecuteCatalog(
				...
				[]string{"fixture-test"},
				"",
				map[string]any{"fixture": tc.fixture},
				evidence.Bundle{PipelineOutputs: map[string]any{
					tc.pipeline: map[string]any{
						"workflowId": tc.name,
						"output":     map[string]any{"value": tc.name},
					},
				}},
			)
			require.NoError(t, err)
