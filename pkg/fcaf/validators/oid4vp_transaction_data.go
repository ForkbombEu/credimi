	...
	for index, rawTransactionData := range transactionData {
		...
		for credentialIndex, rawID := range ids {
			...
			if _, found := credentialIDs[id]; !found {
				return Result{Status: StatusFail, Message: fmt.Sprintf("transaction_data[%d].credential_ids[%d] %q does not match a DCQL credential ID", index, credentialIndex, id)}
			}
		}
	}

	return Result{Status: StatusPass, Message: "all transaction_data credential_ids match requested DCQL credentials"}
}
