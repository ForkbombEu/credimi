...
maestro --device emulator-5580 test all-preconditions.yaml \
  ...
  -e DCQL_ALLOWED_CLAIM_PATH_COMPONENTS_PRESENTATION_URL='<fresh request with string, null, and integer claim paths>' \
...
- `pipeline.dcql.allowed-claim-path-components`: `dcql-allowed-claim-path-components.yaml`
