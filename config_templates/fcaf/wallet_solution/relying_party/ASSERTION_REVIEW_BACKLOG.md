...
- [ ] `WS_RP_SM_SessionEncryption__007` (blocked: beta Capture rejects any `client_metadata` override with `direct_post.jwt`, so no encrypted-response request can advertise only A128GCM)
- [ ] `WS_RP_SM_SessionEncryption__008` (blocked: same override restriction, so no encrypted-response request can advertise only A256GCM)
