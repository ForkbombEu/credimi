...
- [ ] `WS_RP_SH_Encoding_TextualEncoding_011` (the beta issuer now publishes
  `urn:credimi:degree:1` with the source-required heterogeneous `degrees`
  array, so the former fixture blocker no longer applies; an out-of-range
  index selector scenario over that credential is the remaining work)
